...
import { canBeReApply, hasEffectAtTheEndOfRound, listOfStatus, statusConst, statusInterface, statusName, statusNameType } from "../interfaces/status.js";
    ...
    if (monster.statuses.some((monsterStatus) =>  monsterStatus.name === statusToApply.name) &&
        ...
        updateHistory(instance, {
            ...
            content: { targetMonster: convertMonsterToHistory(monster), 
                statusName: statusToApply.name, nbrRound: 0 }
        })
    } else if (monster.statuses.some((monsterStatus) =>  monsterStatus.name === statusToApply.name) &&
            isStatusFromList(statusToApply.name, canBeReApply)) {
...
const reApplyStatus = () => {
    const cold = (instance: instanceInterface, monster: MonsterFightingInterface, effect: effectInterface) => {        
        const nbrRound =  getStatus(monster, statusName.COLD).nbrRound + effect.power
        monster.statuses.splice(monster.statuses.indexOf(monster.statuses[statusName.COLD]), 1)
        monster.statuses.push(buildStatus(statusName.FROZEN, nbrRound)); 

        updateHistory(instance, {
            context: historyContextEnum.STATUS,
            content: { targetMonster: convertMonsterToHistory(monster), 
                statusName: statusName.COLD, nbrRound: -1 }
        })

        updateHistory(instance, {
            context: historyContextEnum.STATUS,
            content: { targetMonster: convertMonsterToHistory(monster), 
                statusName: statusName.FROZEN, nbrRound: nbrRound }
        })
    }
...
const isStatusFromList = (status: statusNameType, statusList: listOfStatus): boolean => {
...
const getStatus = (monster: MonsterFightingInterface, statusName: statusNameType): statusInterface => {
