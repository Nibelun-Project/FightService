...
const deathCheckActionTaget = (instance: instanceInterface, actionsByTarget: actionInterface): boolean => {
...
const deathCheckMonster = (instance: instanceInterface, monster: MonsterFightingInterface): boolean => {
    if (monster.stats[monsterStatsEnum.HP] <= 0) {
...
const _killActionTaget = (instance: instanceInterface, actionsByTarget: actionInterface) => {
    const monster = getPlayerByID(actionsByTarget.targetInfo.targetedPlayerID, instance).onBoard[actionsByTarget.targetInfo.spot];
    _killMonster(instance, monster)
};

const _killMonster = (instance: instanceInterface, monster: MonsterFightingInterface) => {
    if (monster.isAlive) updateHistory(instance, {
