...
const _shuffleMonsters = (
	...
	const _shuffleArray = (array: number[]): number[] => {
		...
		for (let l = 1; l <= array.length - 1; l++) {
			if (
				tempMonstersList[l - 1].playerID ===
				tempMonstersList[l].playerID
			)
				_allyPriority(l - 1, l);
		}
...
const _getPlacesOnRound = (
	...
	speedContestTempsList.forEach((tempMonster) => {
		...
		const count = speedContestTempsList.filter((speedContest) => {
			if (
				speedContest.monster.id != tempMonster.monster.id && //2.1 comparison between the same monster
				(speedContest.action.priority < tempMonster.action.priority || //2.2   - the action have a higher priority
					(tempMonster.action.priority ===
						speedContest.action.priority && //2.3.1 - the action priotity is equal
						speedContest.monster.stats[monsterStatsEnum.SPEED] >
							tempMonster.monster.stats[
								monsterStatsEnum.SPEED
							]) || //2.3.2 - the speed stat is different
					(tempMonster.action.priority ===
